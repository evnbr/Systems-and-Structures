...
<body class="loading">
  ...
  <nav>
<!--     <button id="getstops">Get Stops</button>
 -->    <button id="togglestops">Toggle Stops</button>
  ...
  <script>
    ...
    mapObj = {
      ...
      zoom: function(zoom) {
        ...
        var newW = 300*zoom;
        latScale.range( [ newW, 0    ] );
        lonScale.range( [ 0   , newW ] );
        ...
        $(".map").css({"width": newW, "height": newW });
      },
    ...
    buttons = {
      setup: function() {
        //$('#getstops').on('click', getStopsNearby);

        $('#togglestops').on('click', mapObj.togglestops);


        $('#map')[0].addEventListener('mouseover', function(e){
          if (e.target.className.baseVal.indexOf("stop") === -1) return;
    ...
    tooltip = {
      ...
      show: function(txt,x,y) {
        ...
        this.tip.transition().attr("cx", x).attr("cy", y);
      }
    }
