...
module.exports = {    
  ...
  getShape: function(agency_key, shape_id, cb) {
    ...
    ShapeArr.findOne(query).exec(function(error,shape){
      cb(error, shape.shape_pt_arr);
    });
  ...
  getStopsByDistance: function(lat, lon, radius, cb){
    ...
    Stop
      //.where('loc')
      //.near(lon, lat).maxDistance(radiusInDegrees)
      .find() // -eb hacky added to find all without limit of 100
      .exec(function(e, results){
        console.log("gonna return "+results.length+" stops");
        cb(e, results);
      });
  },

  // Get Stops at this time -Eb
  // ----------------------

  getStopsAtThisTime: function(agency_key, time, cb){
    console.log("getting stops at time "+ time)
    var query = {
        agency_key: agency_key,
        departure_time: time
      };
    StopTime
      ...
      .exec(function(e, stopTimes){
        if(stopTimes.length){
          ...
          cb(new Error('No times available for this stop on this date'), null);
        }
      });
  ...
  getAllTrips: function(agency_key, cb){
    var query = {
      agency_key: agency_key,
    }
    Trip
      ...
      .exec(function(e, trips){
        if(trips.length){
          ...
          cb(new Error('No trips for this date'), null);
        }
    });
  },
