...
function downloadGTFS(task, cb){
  ...
  function importFiles(cb){
    ...
    async.forEachSeries(GTFSFiles, function(GTFSFile, cb){
      var filePath = downloadDir + '/' + GTFSFile.fileNameBase + '.txt';
      if(GTFSFile){
        console.log(agency_key + ': ' + GTFSFile.fileNameBase + ' Importing data');
        db.collection(GTFSFile.collection, function(e, collection){
          if (fs.existsSync(filePath)) {
            csv()
              .from.path(filePath, {columns: true}) // changed fromPath to from.path
              .on('record', function(line, index){ // switched 'record' to 'data'
                 //remove null values
                for(var key in line){
                  if(line[key] === null){
                    delete line[key];
            ...
            console.log(agency_key + ': --> ' + GTFSFile.fileNameBase + ' is not available');
            cb();
          }
        });
      }
